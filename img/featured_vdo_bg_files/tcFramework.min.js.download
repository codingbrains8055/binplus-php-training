...
window.onmessage=LR45.handleSuccess;var codeVersion="codeVersion="+encodeURIComponent(v3Lander&&v3Lander.codeVersion||Date.now());var postToServerHtmName;var postToServerHtmPath;if(site.noJSHosting){var qS=encodeURI("url="+site.clientStaticUrl);postToServerHtmName=site.JSDebugMode?"postToServerNoJS.htm":"postToServerNoJS.min.htm";postToServerHtmPath=postToServerHtmName+"?"+qS;LR45.putItemToP2sStorage(urls.vanityURL,"/tagserver/"+postToServerHtmName+"?"+qS);LR45.putItemToP2sStorage(urls.chatRouterVanityDomain,"/chatrouter/postToServer/"+postToServerHtmName+"?"+qS);LR45.putItemToP2sStorage(urls.cobrowseURL,"/cobrowse/"+postToServerHtmName+"?"+qS);LR45.putItemToP2sStorage(urls.one2ManyVanityDomain,"/chatrouter/postToServer/"+postToServerHtmPath);LR45.putItemToP2sStorage(urls.mediaRootURL,"/tagserver/"+postToServerHtmName+"?"+qS);}else{postToServerHtmName=site.JSDebugMode?"postToServer.dev.htm":"postToServer.min.htm";postToServerHtmPath=postToServerHtmName+"?"+codeVersion;LR45.putItemToP2sStorage(urls.vanityURL,"/tagserver/"+postToServerHtmPath);LR45.putItemToP2sStorage(urls.chatRouterVanityDomain,"/chatrouter/postToServer/"+postToServerHtmPath);LR45.putItemToP2sStorage(urls.one2ManyVanityDomain,"/chatrouter/postToServer/"+postToServerHtmPath);LR45.putItemToP2sStorage(urls.cobrowseURL,"/cobrowse/"+postToServerHtmPath);LR45.putItemToP2sStorage(urls.mediaRootURL,"/tagserver/"+postToServerHtmPath);}
return true;};LR45.putItemToP2sStorage=function(url,p2pPath){var parsedUrl=parseUrl(url);LR45.p2sStorage[parsedUrl.host]=parsedUrl.host+p2pPath;};LR45.handleSuccess=function(event){if(!event.data)return false;var data;if(typeof(event.data)=="string"){data=LR45.convertStrToArray(event.data);}else{data=event.data;}
if(data&&data[0]){var id=data[0];var context=LR45.getContext(id);if(context){context.origin=event.origin;var cmd=data[1];context.id=id;if(data.length>2){context.responseStatus=data[2];context.responseHeaderCacheControl=data[3];context.contentType=data[4];if(context.contentType.indexOf("application/json")>-1){try{context.data=MI8.JSON.parse(cmd);}catch(e){try{window.eval("("+cmd+")");}catch(e){let12("LR45.handleSuccess:"+cf21(e));}}}else if(context.contentType.indexOf("image")>-1){}else{context.data=cmd;}}else{try{if(cmd&&cmd.indexOf("{")==0){context.data=MI8.JSON.parse(cmd);}else{context.data=cmd;context.eval=true;}}catch(e){context.data=cmd;context.eval=true;}}
...
this.addPageToLH();};LE12.prototype.overridePage=function(){var orID=Inq.overridePageID;if(orID>0){this.page=(this.pages[orID]?this.pages[orID]:null);}else if(!v3Lander.isRplMode){this._resolvePage();}};LE12.prototype.restorePagesByIds=function(){var visitedPages=this.lh.map(function(element){return element.id});visitedPages.push(this.page?this.page.id:0);if(visitedPages.length>0){var data={pages:visitedPages,siteID:getSiteID(),url:encodeURIComponent(document.URL),codeVersion:v3Lander.codeVersion};ROM.post(urls.resolvePageURL,data,null,null,function(context){if(context.data&&context.data.pages){this.pages=context.data.pages;}}.bind(this));}};LE12.prototype._resolvePage=function(){var pages=this.pages;var pageURL=win.location.href;for(var i in pages){var re=pages[i].re;if(re===""){continue;}
...
this._init=true;return this;};GR34.prototype.getChatRouterURL=function(){return Inq.one2ManyChat===true?urls.one2ManyVanityDomain:urls.chatRouterVanityDomain;};GR34.prototype.ask=function(url,dataJSON,bust){var rand=(Math.round(Math.random()*1000000000123)).toString(36);var crHost=this.getChatRouterURL()+url+"?";for(var name in dataJSON){crHost+=((crHost.indexOf("?")==-1)?"?":"&")+name+"="+dataJSON[name];}
...
return false;};X43.prototype.agrty=function(idx,callThisNumber){if(CM.cleaningDetected){this.showDisabledIcon();return;}
var c2c=this.getC2C(idx);if(!c2c){log("can't find c2c "+idx);return;}
